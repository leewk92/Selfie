...
public class SafeFaceDetector extends Detector<Face> {
  ...
  /**
   * Determines whether the supplied image may cause a problem with the underlying face detector. If
   * it does, padding is added to the image in order to avoid the issue.
   */
  @Override
  public SparseArray<Face> detect(Frame frame) {
    ...
    if (height > (2 * kDimensionLower)) {
      ...
      final double multiple = (double) height / (double) kDimensionLower;
      final double lowerWidth = Math.floor((double) width / multiple);
      ...
      final double multiple = (double) width / (double) kDimensionLower;
      final double lowerHeight = Math.floor((double) height / multiple);
  ...
  /**
   * Creates a new frame based on the original frame, with additional width on the right to increase
   * the size to avoid the bug in the underlying face detector.
   */
  ...
  private Frame padFrameBottom(Frame originalFrame, int newHeight) {
    ...
    ByteBuffer origBuffer = originalFrame.getGrayscaleImageData();
    final int origOffset = origBuffer.arrayOffset();
    byte[] byteArray = origBuffer.array();
    ...
    for (int y = 0; y < height; ++y) {
      ...
      System.arraycopy(byteArray, origStride, paddedBytes, paddedStride, width);
    }
  ...
  static public boolean hasFace(Context context, Bitmap bitmap) {
